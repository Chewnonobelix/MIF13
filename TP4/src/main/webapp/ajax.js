...
function traiteXML(XMLDoc, id){
    ...
    var messages = "";
    var nb_messages = XMLDoc.getElementsByTagName('Message').length;

    if(nb_messages > 0){
        
        for(var i = 0; i < nb_messages; i++){

            messages += '<tr id="message">'
            messages += '<td>' + XMLDoc.getElementsByTagName('heure')[i].firstChild.data + '</td>'
            messages += '<td>' + XMLDoc.getElementsByTagName('Auteur')[i].firstChild.data + '</td>';
            messages += '<td>' + XMLDoc.getElementsByTagName('Texte')[i].firstChild.data + '</td>';
            messages += '</tr>';
        }

        //on ajoute les messages au document
